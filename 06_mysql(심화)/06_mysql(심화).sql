-- 문제1. sqldb에서 usertbl 테이블에서 다음 내용들을 확인하세요
-- ○ usertbl의 내용 확인
use sqldb;
select * from usertbl;
...
-- 문제2. usertbl의 addr 컬럼에 대해 idx_usertbl_addr이름으로 인덱스를 만들고, 인덱스 목록을 확인하세요.
create index idx_usertbl_addr on usertbl(addr);
show index from usertbl;

-- 문제3. usertbl의 상태를 출력하여 인덱스의 내용이 만들어졌는지 확인하고, 내용이 없다면 실제 적용되도록 한 후, 인덱스의 크기를 확인하세요.
show table status like 'usertbl';
analyze table usertbl;
show table status like 'usertbl';

-- 문제4. usertbl에 대해 다음을 처리하세요.
...
-- 문제6. usertbl에서 앞에서 만든 인덱스를 삭제하세요.
show index from usertbl;

drop index idx_user_addr on usertbl;
drop index idx_usertbl_name_birthYear on usertbl;

-- 문제7. 다음과 같이 실습 데이터베이스 및 사용자를 생성하고, 해당 데이터베이스에 대해 모든 권한을 부여하세요.
